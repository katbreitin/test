...
module CLOUD_TYPE_ALGO_MODULE 
   ...
   type cloud_type_rtm_type
      ...
      real, allocatable:: rad_ch38_bb_prof (:)
      ...
      real :: bt_ch38_3x3_max
      real :: bt_ch38_3x3_std
      real :: Covar_Ch27_Ch31_5x5
      real :: Covar_Ch27_Ch38_5x5
      real :: ref_ch6_clear
      real :: rad_ch27_atm_sfc
      real :: rad_ch31_atm_sfc
      real :: rad_ch38_atm_sfc
      real :: bt_ch31_atm_sfc
      real :: bt_ch32_atm_sfc
      real :: bt_ch38_atm_sfc
      real :: emiss_tropo_ch31     
      real :: emiss_tropo_ch38     
      real :: beta_11um_12um_Tropo
      real :: beta_11um_133um_Tropo
      real :: beta_104um_12um_Tropo
      real :: beta_104um_133um_Tropo
...
contains
   ...
   subroutine CLOUD_TYPE_PIXEL ( inp , ctype , diag_out, ABI_Use_104um_Flag, ice_prob_out , force_ice , force_water ) 
      ...
      call GET_ICE_PROBABILITY (inp &
         , ice_prob , is_cirrus , is_water , t_cld , z_cld, ABI_Use_104um_Flag ) 
      ...
      if ( (ice_prob > 0.5 .or. force_ice_phase) .and. .not. force_water_phase ) then
        if (ABI_Use_104um_Flag) then
          call DETERMINE_TYPE_ICE ( inp % Rtm % emiss_tropo_ch38 &
            , inp % sat % bt_ch38 &
            , inp % rtm % beta_104um_12um_Tropo &
            , inp % rtm % beta_104um_133um_Tropo &
      ...
      if ( experimental_on ) then
        ...
        if (ABI_Use_104um_Flag) then
          call OVERLAP_TEST ( inp % sat % bt_ch38 &
                        , inp % sat % bt_ch32 &
                        , inp % sat % ref_ch6 & 
                        , inp % geo % sol_zen &
                        , inp % geo % sat_zen &
   ...
   ! ------------------------------------------------------------------------------------
   subroutine GET_ICE_PROBABILITY (inp , ice_prob , is_cirrus , is_water , t_cld , z_cld, ABI_Use_104um_Flag ) 
      implicit none
      type ( cloud_type_input_type) , intent(in) :: inp
      ...
      if (ABI_Use_104um_Flag) then
        if ( .not. inp % sat % chan_on ( 38) ) then
          print*, ' channel 38 (10.4 micron) was not read '
      ...
      if (ABI_Use_104um_Flag) then
        ...
        if ( inp % sat % chan_on ( 27) ) then
          ...
          call HEIGHT_H2O_CHANNEL ( &
             inp % sat % rad_ch38 &
             , inp % rtm % rad_ch38_bb_prof &
             , inp % rtm % rad_ch38_atm_sfc &
             , inp % sat % rad_ch27 &
             , inp % rtm % rad_ch27_bb_prof &
             , inp % rtm % rad_ch27_atm_sfc &
             , inp % rtm % Covar_Ch27_Ch38_5x5 &
             , inp % rtm % tropo_lev &
             , inp % rtm % sfc_lev &
             , inp % rtm % t_prof &
             , inp % rtm % z_prof &
      ...
      end if  !--- 10.4 um check
     
      if (ABI_Use_104um_Flag) then

        !---- if no water vapor, use the atmos-corrected 10.4 micron
        if ( t_cld == MISSING_VALUE_REAL .and. inp % sat % chan_on ( 38)  ) then
          call HEIGHT_OPAQUE ( &
            inp % sat % rad_ch38 &
            , inp % rtm % rad_ch38_bb_prof &
            , inp % rtm % tropo_lev &
      ...
      if (ABI_Use_104um_Flag) then
        !--- last resort, use raw 10.4 micron brightness temperature
        if ( t_cld == MISSING_VALUE_REAL) t_cld = inp % sat % bt_ch38    
      else
      ...
      if ( t_cld > 240.0 ) then
         ...
         if (ABI_Use_104um_Flag) then
           ...
           if (  inp % sat % chan_on(29)  &
            .and. inp % sat % chan_on ( 38 ) ) then
            
             if (  (inp % sat % bt_ch29 - inp % sat % bt_ch38 ) <  BT_85_MINUS_BT_11_TEST ) is_water = .true.
           end if      
         else
           ! -  8.5-11 test
           if (  inp % sat % chan_on(29)  &
      ...
      if (ABI_Use_104um_Flag) then
        ...
        !---- don't detect cirrus if very high 10.4 um std deviation
        if ( inp % rtm % bt_ch38_3x3_std < BT_11UM_STD_CIRRUS_THRESH .and. .not. is_water) then
      
          ! - split window test
          if ( inp % sat % chan_on ( 38 ) &
            .and. inp % sat % chan_on ( 32 )) then
                     
            if ( inp % rtm % bt_ch38_atm_sfc <= 265.0 ) then
               h2o_correct = FMFT_COLD_OFFSET
            else 
               h2o_correct = (inp % rtm % bt_ch38_atm_sfc -inp % rtm %  bt_ch32_atm_sfc) &
                  & * (inp % sat % bt_ch38 -260.0) &
                  & / (inp % rtm % bt_ch38_atm_sfc -260.0)
            end if
            
            fmft = inp % sat % bt_ch38 - inp % sat % bt_ch32 - h2o_correct
          ...
          if ( inp % sat % chan_on ( 27 ) &
            ...
            if ( inp%rtm%Covar_Ch27_Ch38_5x5 > 1.5 &
