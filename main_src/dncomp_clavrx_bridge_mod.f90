...
!ccm      dcomp_input % cloud_type % d  = cld_type
         ...
         if (run_nlcomp) then
            ! debug_mode_user set to 0 because causing seg fault on viirs gfortran, 
            ! ifort works fine. Andi you can change it, but verify (Denis B.)
            call nlcomp_array_loop_sub (dcomp_input, dncomp_output, debug_mode_user = 0) !debug_mode_user )
         else  
         ...
         ! === POPULATE CLAVR-X VARIABLES FROM PIXEL_COMMON_MOD
!ccm
