!$Id: laszlo_insolation_mod.f90 3371 2019-06-20 14:09:31Z awalther $
!--------------------------------------------------------------------------------------
! Clouds from AVHRR Extended (CLAVR-x) 1b PROCESSING SOFTWARE Version 6.0
!
! NAME: laszlo_insolation_mod.f90 (src)
!       laszlo_insolation_mod (program)
!
   ...
   subroutine INSOLATION(Line_Idx_Min_Segment,Number_Of_Lines_Read_This_Segment)
      ...
      element_loop: do Elem_Idx = First_Element, First_Element + Number_of_Elements - 1
         line_loop: do Line_Idx = 1, Number_of_Lines
            ...
            Ozone = NWP_PIX%Ozone(Elem_Idx,Line_Idx)  !Dobson
            !--- convert Ozone into atm-cm
            if (Ozone /= Missing_Value_Real4) Ozone = Ozone / 1000.0

            Pwater = NWP_PIX%Tpw(Elem_Idx,Line_Idx)   !g/cm^2 or cm
...
end module LASZLO_INSOLATION_MOD
