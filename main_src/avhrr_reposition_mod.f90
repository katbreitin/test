...
!--- check for bad scans
      ...
      valid_scan_number(jj) = image%scan_number(j)

!--- determine date of scan
      if (image%scan_time_ms(j) >= image%start_time) then
         day = start_dom
         month = start_month
...
!--- determine hour, minute and second
      temp = image%scan_time_ms(j) / 60.0 / 60.0/ 1000.0
...
end module AVHRR_REPOSITION_MOD
