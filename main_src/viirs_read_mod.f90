!$Id: viirs_read_mod.f90 4009 2020-09-23 22:27:34Z dbotambekov $
!--------------------------------------------------------------------------------------
...
!
!         ! - added DNB geo-location for J01 (aka NOAA-20) 5 Feb 2018 AW 
!--------------------------------------------------------------------------------------
module viirs_read_mod
   ...
   type , public :: viirs_data_config
      ...
      logical :: sapf_cloud_mask_on
      logical :: sapf_cloud_type_on
   ...
   type :: cloud_products_str
      logical :: is_mask_read
      logical :: is_phase_read
...
contains
   ...
   subroutine read_viirs_level1b (config, out,  error_out )
      ...
      out % prd % is_mask_read = .false.
      out % prd % is_phase_read = .false.
      ...
      platform = trim(config % file_gmtco_base(7:9))
      ...
      do i_iband = 1 , 5
         out % iband (i_iband) % is_read = .false.
         ...
         call h5readdataset ( trim(config %dir_1b)//trim(file_iband), trim(setname_iband) , offset_iband  , dim_seg_iband , i2d_buffer )
         dim_buf = shape ( i2d_buffer )
         allocate ( invalid_pixel ( dim_buf(1) , dim_buf(2) ) )
         invalid_pixel = i2d_buffer >= scaled_missing
         call h5readdataset ( trim(config %dir_1b)//trim(file_iband), trim(setname_iband_fac) , factors )
         allocate  ( r2d_data  ( dim_buf(1) , dim_buf(2) ))
         r2d_data =  i2d_buffer * factors(1) + factors(2)
      ...
      if ( config %  viirs_cloud_mask_on ) then 
         ...
         if ( n_files /= 0 ) then
           ...
           !                    + 4. * log2int(btest( i2d_buffer , 2 ) )
            ...
            out % prd % is_phase_read = .true.

            deallocate ( i2d_buffer )
            call MESG ("Auxiliary IDPS Mask read in successful",level = verb_lev %DEFAULT) 

         else 
            out % file_exists % iicmo_file_exists = .false.
            out % prd % is_mask_read = .false.
            out % prd % is_phase_read = .false.
            call MESG ("Auxiliary IDPS Mask read in failed",level = verb_lev % DEFAULT)
...
! ---------------------------------------------------------------------------------------
    ...
    subroutine CONVERT_RAD_2_SOL_REF_DNB ( radiance , solzen , sun_earth_distance, missing_value , reflectance )
