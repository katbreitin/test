...
!$Id: nb_cloud_mask_sapf_bridge_module.f90 3626 2019-12-10 18:24:33Z dbotambekov $
!-------------------------------------------------------------------------------
!
! PURPOSE: 
!
...
!   INTEGER(LONG) :: Sfc_Idx_NWP
   ...
   character (len=1020):: NB_file_LUT,NB_prior_file_LUT
...
!    
   ...
   line_loop: do Line_Idx = 1, Num_Line
      elem_loop: do  Elem_Idx = 1, Num_Elem
        ...
        else ! --- added by Denis B. Asked by Land team, shouldn't affect ECM
             ! --- simple geometrical land glint if glint angle < 40 degrees
          if (Glintzen(Elem_Idx,Line_Idx) < GLINT_ZEN_THRESH) &
                        Glint_Mask(Elem_Idx,Line_Idx) = sym%YES
...
!-------------------------------------------------------------------------------
   ...
   subroutine SET_INPUT(i,j, Ctxt)
      ...
      ! For current GOES, we need the WV channel
      IF ((CHN_FLG(9) == sym%NO) .and. &
          (CHN_FLG(10)== sym%YES)) THEN
            Input%Chan_On_67um = CHN_FLG(10)
            Input%Bt_67um = Chn10BT(i,j)
      ENDIF
...
!      CALL OBTAIN_SFCTEMP(Tsfc_SST, Tsfc_NWP, Input%Land_Class, Tsfc)
   ...
   subroutine SET_OUTPUT(i,j)
      ...
      Cld_Mask_QF(i,j) = Output%Cld_Mask_QF
...
!--- compute median
  ...
  enddo
   ...
   SUBROUTINE LOAD_ECM_DATA(Ctxt)

        TYPE(CLOUD_MASK_EN_Ctxt), intent(in) :: Ctxt
      ...
      CALL NFIP_CloudMask_FileName2(Ctxt%CLOUD_MASK_Src1_T00, NB_prior_file_LUT)
      CALL Convert_Char_C2Fortran(NB_file_LUT)  
        ...
        CALL NFIA_Sat_L1b_BrtTemp(Ctxt%SATELLITE_DATA_Src1_T00, COMMON_RESOLUTION, CHN_ABI10, Chn10BT)   !7.34
