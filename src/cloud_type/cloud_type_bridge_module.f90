...
contains
   ...
   subroutine cloud_type_bridge
      ...
      elem_loop: do  j = 1,Image%Number_Of_Lines_Read_This_Segment
         line_loop: do i = 1, Image%Number_Of_Elements  
            ...
            if (CLDMASK%Dust_Mask (i,j) == 1 ) then
               cld_type (i,j ) = et_cloud_type % DUST
               cycle
            end if

            if (CLDMASK%Smoke_Mask (i,j) == 1 ) then
               cld_type (i,j ) = et_cloud_type % SMOKE
               cycle
            end if

            if (CLDMASK%Fire_Mask (i,j) == 1 ) then
            ...
            call CLOUD_TYPE_PIXEL  ( type_inp, ctype , diag_out, ABI_Use_104um_Flag, &
   ...
   subroutine POPULATE_INPUT ( i, j , type_inp)
      ...
      Nwp_Lon_Idx = NWP_PIX%I_Nwp( i , j )
      Nwp_Lat_Idx = NWP_PIX%J_Nwp( i , j )
      ...
      if (sensor % chan_on_flag_default(38) == 1 ) type_inp % sat % rad_ch38 = ch(38) % rad_toa ( i,j )
      if (sensor % chan_on_flag_default(38) == 1 ) type_inp % sat % bt_ch38 =  ch(38) % bt_toa  ( i,j )
      ...
      if (ABI_Use_104um_Flag) then
        if (sensor % chan_on_flag_default(38) == 1 )  then
          ...
          allocate ( type_inp % rtm % rad_ch38_bb_prof (n_rtm_prof ) &
             , source = Rtm(Nwp_Lon_Idx,Nwp_Lat_Idx)%d(Vza_Idx)%ch(38)%Rad_BB_Cloud_Profile)

          type_inp % rtm % bt_ch38_3x3_max    = ch(38)%Bt_Toa_Max_3x3( i,j )
          type_inp % rtm % bt_ch38_3x3_std    = ch(38)%Bt_Toa_Std_3x3( i,j )
          type_inp % rtm % rad_ch38_atm_sfc   = ch(38)%Rad_Toa_Clear(i,j)
          type_inp % rtm % bt_ch38_atm_sfc    = ch(38)%Bt_Toa_Clear( i,j )
          type_inp % rtm % emiss_tropo_ch38   = ch(38)%Emiss_Tropo( i,j )
          if (sensor % chan_on_flag_default(27) == 1 )  then
            type_inp % rtm % Covar_Ch27_Ch38_5x5 = Covar_Ch27_Ch38_5x5( i,j )
          endif
          if (sensor % chan_on_flag_default(32) ==1 )  then
            type_inp % rtm % Beta_104um_12um_Tropo  = Beta_104um_12um_Tropo_Rtm( i,j )
            type_inp % rtm % bt_ch32_atm_sfc       = ch(32)%Bt_Toa_Clear( i,j )
          endif
          if (sensor % chan_on_flag_default(33) ==1 )  then
            type_inp % rtm % Beta_104um_133um_Tropo = Beta_104um_133um_Tropo_Rtm( i,j )
          endif
        endif
      else
        if (sensor % chan_on_flag_default(31) == 1 )  then
          ...
          type_inp % rtm % bt_ch31_3x3_max    = ch(31)%Bt_Toa_Max_3x3( i,j )
          type_inp % rtm % bt_ch31_3x3_std    = ch(31)%Bt_Toa_Std_3x3( i,j )
      ...
      if (sensor % chan_on_flag_default(27) == 1) then
         type_inp % rtm % bt_ch27_3x3_max    = ch(27)%Bt_Toa_Max_3x3( i,j )
   ...
   subroutine DEALLOCATE_INP ( type_inp)
      type ( cloud_type_input_type) :: type_inp
       ...
       if ( allocated( type_inp % rtm % rad_ch31_bb_prof)) deallocate ( type_inp % rtm % rad_ch31_bb_prof )
       if ( allocated( type_inp % rtm % rad_ch38_bb_prof)) deallocate ( type_inp % rtm % rad_ch38_bb_prof )
