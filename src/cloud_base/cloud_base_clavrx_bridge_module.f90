...
!ynoh (cira/csu) for ccl mode 3
     Output%Pc_Lower_Base =>  null()
     Output%Zc_Base_Qf => null()
     Output%Geo_Thickness =>  null()
 ...
 subroutine SET_OUTPUT()
   Output%Zc_Base => BASE%Zc_Base
   Output%Pc_Base => BASE%Pc_Base
   Output%Tc_Base => BASE%Tc_Base
   Output%Zc_Base_Qf => BASE%base_Quality_Flag
!ynoh (cira/csu) for ccl mode 3
   Output%Pc_Lower_Base => BASE%Lower_Base_Pc
   Output%Geo_Thickness => BASE%Geo_Thickness
