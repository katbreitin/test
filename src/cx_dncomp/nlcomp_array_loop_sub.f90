...
subroutine nlcomp_array_loop_sub ( input , output, debug_mode_user )
  ...
  use univ_kind_defs_mod, only: f4,  i2
   ...
   real( f4 ) :: trans_ozone ( N_CHAN )
   real( f4 ) :: trans_unc_ozone ( N_CHAN )
   real( f4 ) :: trans_rayleigh ( N_CHAN )
   real( f4 ) :: trans_wvp ( N_CHAN )
   real( f4 ) :: trans_unc_wvp ( N_CHAN ) = 0.0
   real( f4 ) :: trans_total ( N_CHAN )
   real( f4 ) :: assumed_tpw_error
   real( f4 ), parameter :: ozone_coeff (3)    = [ -0.000606266 , 9.77984e-05,-1.67962e-08 ]
   real ( f4 ) :: refl_toc(N_CHAN)
   real ( f4 ) :: alb_sfc(N_CHAN)
   real ( f4 ) :: alb_unc_sfc(N_CHAN)
   real ( f4 ) :: rad_to_refl_factor
   real( f4 )   :: refl_toa = -999.
