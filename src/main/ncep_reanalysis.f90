! $Id: ncep_reanalysis.f90 4052 2020-11-19 13:55:22Z awalther $
!--------------------------------------------------------------------------------------
...
private:: READ_DATA_1D, READ_DATA_2D, READ_DATA_3D
...
!--- reorder
do level = 1, nlevels
  NWP%z_prof(level,:,:) = ncep_geopothgt(:,:,nlevels+1-level)
  NWP%t_prof(level,:,:) = ncep_airtemp_profile(:,:,nlevels+1-level)
enddo

n = size(ncep_relhum_profile,3)
!rh_prof_nwp(:,:,nlevels-n+1:nlevels) = ncep_relhum_profile(:,:,n:1:-1)
do level = nlevels-n+1,nlevels
 NWP%rh_prof(level,:,:) = ncep_relhum_profile(:,:,nlevels-level+1)
enddo
...
!------------------------------------------------------------
nwp%psfc = ncep_psfc
nwp%tmpair = ncep_airsfc
nwp%tpw = ncep_tpw / 10.0         !now in cm
nwp%zsfc = 0.0    ! this is missing
nwp%rhsfc = ncep_rhsfc
nwp%land = int(ncep_land,kind=int1)
nwp%t_trop = ncep_t_trop
nwp%p_trop = ncep_p_trop
...
!--- Convert those things on the T62 Gaussian grid to the 2.5x2.5 grid
  do j = 1, nwp%nlat
    do i = 1, nwp%nlon
      ...
      nwp%tmpsfc(i,j) = ncep_skint(lon_index_gauss(i),lat_index_gauss(j))
      nwp%weasd(i,j) = ncep_weasd(lon_index_gauss(i),lat_index_gauss(j))
      nwp%u_wnd_10m(i,j) = ncep_u_wnd_10m(lon_index_gauss(i),lat_index_gauss(j))
      nwp%v_wnd_10m(i,j) = ncep_v_wnd_10m(lon_index_gauss(i),lat_index_gauss(j))
    enddo
  enddo
