...
module dnb_retrievals_mod
   ...
   ! - paramaters
   ! - physical
   double precision , parameter, private :: MEAN_EARTH_SUN_DIST = 149598022.6071
   double precision , parameter, private :: MEAN_EARTH_MOON_DIST = 384400.0
...
!---  allocate ( ref_chdnb_lunar(num_pix,num_elem) )
   ...
   !-------------------------------------------------------------------
   ! computes lunar_irrad_dnb
   !-------------------------------------------------------------------
   subroutine compute_lunar_irrad_dnb ( & 
                 year &
               , month &
               , day_of_month &
               , start_time &
      ...
      ! - output
      real, intent ( out ) :: lunar_irrad_dnb 
      ...
      hour_fraction = minute / 60.0
      hour = floor(start_time/1000./60./60.)
      yyyymmddhh = year * 1000000 + month*10000+ day_of_month *100 + int(hour)
   ...
   end subroutine compute_lunar_irrad_dnb

   !-------------------------------------------------------------------
   !
   !-------------------------------------------------------------------
   subroutine compute_lunar_ref_dnb(lunar_irrad_dnb,solzen,lunzen,rad_chdnb_input,ref_chdnb_lunar)

      real, intent(in):: lunar_irrad_dnb
      real , intent(in) :: solzen(:,:)
      real , intent(in) :: lunzen(:,:)
      real , intent(in) :: rad_chdnb_input(:,:)
      real, intent ( out ) :: ref_chdnb_lunar(:,:)
   ...
   end subroutine compute_lunar_ref_dnb
