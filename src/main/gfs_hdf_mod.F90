...
module GFS_HDF_MOD
  ...
  use CX_HDF4_MOD, only: hdf_sds_reader&
  ,READ_HDF_GLOBAL_ATTRIBUTE_NUM &
  , READ_HDF_GLOBAL_ATTRIBUTE_STR &
  ,OPEN_FILE_HDF_READ  &
  , CLOSE_FILE_HDF_READ
...
contains
  ...
  subroutine READ_GFS_DATA(Nwp_Data_Type, &
    ...
    integer, intent(in) :: Start_year, Start_jday, end_year, end_jday
    ...
    logical :: valid_nwp_data_type

    !-- check Nwp_Data_Type
    valid_nwp_data_type = &
    Nwp_Data_Type .EQ. 1 .OR. &
    Nwp_Data_Type .EQ. 3 .OR. &
    Nwp_Data_Type .EQ. 4 .OR. &
    Nwp_Data_Type .EQ. 5 .OR. &
    Nwp_Data_Type .EQ. 6 .OR. &
    Nwp_Data_Type .EQ. 7 .OR. &
    Nwp_Data_Type .EQ. 8

    if (.not. valid_nwp_data_type) then
  ...
  subroutine INTERPOLATE_2D(X,Missing1,Missing2,Temp)
    ...
    do i1 = 1, Ndim1
      do i2 = 1, Ndim2
        none_is_missing = &
        (Temp2d_Nwp_1(i1,i2) /= Missing1) .and. &
        (Temp2d_Nwp_2(i1,i2) /= Missing1) .and. &
        (Temp2d_Nwp_1(i1,i2) /= Missing2) .and. &
        (Temp2d_Nwp_2(i1,i2) /= Missing2)
        if (none_is_missing) then
  ...
  subroutine INTERPOLATE_3D(X,Missing1,Missing2,Temp)
    ...
    do i1 = 1, Ndim1
      do i2 = 1, Ndim2
        do i3 = 1, Ndim3
          none_is_missing = (Temp3d_Nwp_1(i1,i2,i3) /= Missing1) .and. &
          (Temp3d_Nwp_2(i1,i2,i3) /= Missing1) .and. &
          (Temp3d_Nwp_1(i1,i2,i3) /= Missing2) .and. &
          (Temp3d_Nwp_2(i1,i2,i3) /= Missing2)
