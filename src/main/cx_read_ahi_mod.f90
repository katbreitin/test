...
!using one CGMS routine (geos_transform_pix.c for all sensors)
            ...
            call  possol ( day_of_year ,  hour_frac  , real(lonx) &
             ...
             geo % satzen (ii,jj) = sensor_zenith ( GEO_ALTITUDE, real(sub_lon) &
               ,real(sub_lat) &
               ,real(lonx) , real(latx) )

              geo % sataz (ii,jj) = sensor_azimuth (  real(sub_lon),real(sub_lat) &
              ,real(lonx) , real(latx) )

              geo % relaz(ii,jj) = relative_azimuth ( geo % solaz (ii,jj)  &
              , geo % sataz (ii,jj))

              geo % glintzen(ii,jj) = glint_angle ( geo % solzen (ii,jj) &
                  ,geo % satzen (ii,jj) &
                  ,   geo % relaz(ii,jj) )
