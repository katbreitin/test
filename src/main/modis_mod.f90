!$Id: modis_mod.f90 3897 2020-06-30 18:38:16Z dbotambekov $
!--------------------------------------------------------------------------------------
...
module MODIS_MOD
    ...
    !   use PIXEL_ROUTINES_MOD,only: qc_modis
        ...
        integer(kind=int2), parameter, private:: fill_value = 32767_int2
...
subroutine DETERMINE_MODIS_CLOUD_MASK_FILE(Modis_1b_Name, Dir_Modis_Cloud_Mask, Auxiliary_Cloud_Mask_File_Name)
    ...
    if (Sensor%Spatial_Resolution_Meters == 1000) then
      if (trim(Sensor%Platform_Name)  == 'AQUA') then
         ...
         Search_String = "MOD35_L2"//trim(Search_String(9:nc))//"*"
      endif
    else
      if (trim(Sensor%Platform_Name)  == 'AQUA') then
...
error_check: do while (Status_Flag == 0 .and. Iend == 0)
      ...
      do iline = 1,ny_local_temp-1
          if (lat_out(nx_local/2,iline+1) > lat_out(nx_local/2,iline)) then
                  asc_des_flag(iline) = 0_int1
          else
                  asc_des_flag(iline) = 1_int1
          endif
      enddo
...
error_check: do while (Error_Status == 0 .and. End_Flag == 0)
    ...
    if (trim(Sensor%Sensor_Name) == 'MODIS' .or. trim(Sensor%Sensor_Name) == 'MODIS-CSPP') then
       ...
       if (Use_Aux_Flag /= sym%NO_AUX .and. trim(Image%Auxiliary_Cloud_Mask_File_Name) /= "no_file") then
          ...
          if (Error_Status == 0) then
             ...
             if (Seg_Idx == 1) &
                      print *, EXE_PROMPT, MODULE_PROMPT, "Aux MODIS Cloud Mask Read In"
          endif
...
!======================================================================
    ...
    subroutine READ_MODIS_DATE_TIME(Path, File_Name, Start_Year, Start_Day, &
      ...
      Start_Time =  int((hour + (float(minute)/60.0) + ((float(seconds)/60.0/60.0))) * &
      60.0 * 60.0 * 1000.0,kind=int4)
      ...
      End_Time =  int((hour + (float(minute)/60.0) + ((float(seconds)/60.0/60.0))) * &
      60.0 * 60.0 * 1000.0,kind=int4)
      ...
      Start_Year= int(year,kind=int2)
      Start_Day = int(jday,kind=int2)
