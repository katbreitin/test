! $Id: surface_properties_mod.f90 4052 2020-11-19 13:55:22Z awalther $
!--------------------------------------------------------------------------------------
...
real(kind=real4), dimension(0:ntype_sfc-1), public,save:: Ch38_Sfc_Emiss_Umd
...
!  Ch1_Sfc_Alb_Umd = (/0.0300, & !0
   ...
   !--- channel 38 surface emissivity based on SeeBor et al 2006.
   !--- Faked from Ch 31.
   Ch38_Sfc_Emiss_Umd = (/0.985,  &   !0
                     0.965,  &   !1
                     0.961,  &   !2
                     0.963,  &   !3
                     0.963,  &   !4
...
!------------------------------------------------------------------------
! Fill In Hole in White Sky Reflectance
!------------------------------------------------------------------------
subroutine GET_PIXEL_SFC_REFL_FROM_SFC_TYPE()

   integer:: i, j
   do j = 1, Image%Number_Of_Lines_Read_This_Segment
      do i = 1, Image%Number_Of_Elements

         if (Bad_Pixel_Mask(i,j) == sym%YES) cycle
         if (Sfc%Sfc_Type(i,j) < 0) cycle

         if (allocated(ch(1)%Sfc_Ref_White_Sky)) then
           if (ch(1)%Sfc_Ref_White_Sky(i,j) == MISSING_VALUE_REAL4) then
              ch(1)%Sfc_Ref_White_Sky(i,j) = Ch1_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
              if (Sfc%Snow(i,j) /= sym%NO_SNOW) ch(1)%Sfc_Ref_White_Sky(i,j) = Ch1_Snow_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
           endif
         endif

         if (allocated(ch(2)%Sfc_Ref_White_Sky)) then
           if (ch(2)%Sfc_Ref_White_Sky(i,j) == MISSING_VALUE_REAL4) then
              ch(2)%Sfc_Ref_White_Sky(i,j) = Ch2_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
              if (Sfc%Snow(i,j) /= sym%NO_SNOW) ch(2)%Sfc_Ref_White_Sky(i,j) = Ch2_Snow_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
           endif
         endif

         if (allocated(ch(6)%Sfc_Ref_White_Sky)) then
           if (ch(6)%Sfc_Ref_White_Sky(i,j) == MISSING_VALUE_REAL4) then
              ch(6)%Sfc_Ref_White_Sky(i,j) = Ch6_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
              if (Sfc%Snow(i,j) /= sym%NO_SNOW) ch(6)%Sfc_Ref_White_Sky(i,j) = Ch6_Snow_Sfc_Alb_Umd(Sfc%Sfc_Type(i,j))
           endif
         endif
    ...
    i_loop:    do i = 1,Image%Number_Of_Elements
      ...
      if (sfc_emiss_option == 0) then
        ...
        if (Sfc%Sfc_Type(i,j) >= 0 .and. Sfc%Sfc_Type(i,j) < 15) then     !limits depend on Sfc_Type
          ...
          if (Sensor%Chan_On_Flag_Default(38)==sym%YES) Ch(38)%Sfc_Emiss(i,j) = Missing_Value_Real4
...
!--- set to constant until we can verify the above
        ...
        if (Sensor%Chan_On_Flag_Default(38)==sym%YES) Ch(38)%Sfc_Emiss(i,j) = 0.985
      end if
      ...
      if (Sfc%Snow(i,j) /= sym%NO_SNOW) then
        ...
        if ((Sfc%Sfc_Type(i,j) /= sym%EVERGREEN_NEEDLE_SFC) .and. &
          ...
          if (Sensor%Chan_On_Flag_Default(38)==sym%YES) Ch(38)%Sfc_Emiss(i,j) = 0.979
