!$Header: https://svn.ssec.wisc.edu/repos/cloud_team_clavrx/trunk/main_src/iff_clavrx_bridge.f90 3082 2018-12-17 17:53:19Z mfoster $
!--------------------------------------------------------------------------------------
...
!----------------------------------------------------------------------
   subroutine READ_IFF_DATA (segment_number, iff_file, error_out)
      ...
      integer, parameter :: num_chan = 38
      integer , dimension(num_chan) :: modis_chn_list
      integer :: i_band
      integer :: y_start , c_seg_lines
      integer :: iline
      integer :: ch_tmp
      logical, dimension(num_chan) :: is_band_on
      ...
      iff_conf % iff_cloud_mask_on = Use_Aux_Flag /= sym%NO_AUX
      ...
      Image%Scan_Time_Ms(1:c_seg_lines)   = out % geo % scan_time
      ...
      do iline = 1, Image%Number_Of_Lines_Read_This_Segment
        Image%Scan_Number(iline) = y_start + iline - 1
      enddo
      ...
      do i_band = 1 , iff_conf % n_chan
         ...
         elseif ((modis_chn_list(i_band) >= 20 .and. modis_chn_list(i_band) < 26) &
            .or. (modis_chn_list(i_band) > 26 .and. modis_chn_list(i_band) <= 44)) then
            ...
            .or.(modis_chn_list(i_band) >= 33 .and. modis_chn_list(i_band) <= 44))) then
                sounder_flag = .true.
            elseif (trim(Sensor%Sensor_Name) == 'VIIRS-IFF' .and. &
                modis_chn_list(i_band) >= 33 .and. modis_chn_list(i_band) <= 44) then
                sounder_flag = .true.
            endif
