!$Id: level2b_mod.f90 3773 2020-03-12 01:32:25Z heidinger $
!--------------------------------------------------------------------------------------
...
!end interface
 ...
 type, public :: Sds_Struct
  ...
  integer(kind=int4):: Data_Type_HDF
...
subroutine DEFINE_SDS_RANK1(Sd_Id,            &
    ...
    Istatus_Sum = 0
     ...
     Sds%Id_Output = sfcreate(Sd_Id,trim(Sds_Name_Temp),Sds%Data_Type_HDF,Sds%rank,Sds_Dims)
...
!-----------------------------------------------------------------------------------------------
  subroutine READ_SDS_RANK1(Sd_Id,     &
    ...
    if ( Sds%Id_Input < 0 ) then
       print *, "WARNING: Unable to read Level-2 SDS named ",trim(Sds%Variable_Name)
       Sds%Data_Type_HDF = -999
       return
    endif

    Istatus = sfginfo(Sds%Id_Input, Sds_Name_Temp, Sds%rank, Sds_Dims, Sds%Data_Type_HDF, Num_Attrs) + Istatus
    Sds_Start = (/ 0 /)
    Sds_Stride = (/ 1 /)
    Sds_Edges = Sds_Dims
    ...
    elseif (Sds%Data_Type_HDF == DFNT_INT32) then
    ...
    elseif (Sds%Data_Type_HDF == DFNT_FLOAT32) then
       ...
       print *, "data type was ", Sds%Data_Type_HDF
...
!-----------------------------------------------------------------------------------------------
  ...
  Lon_Start = int((Lon_West_Out - Lon_West_In) / Dlon_In + 1,kind=int2)
  Lat_Start = int((Lat_South_Out - Lat_South_In) / Dlat_In + 1,kind=int2)
...
subroutine WRITE_SCALING_ATTRIBUTES(Sds,Istatus_Sum)
    ...
    integer:: sfsnatt
     ...
     if (Sds%Scaling_Type > 0) then
      ...
      if (Sds%Data_Type_HDF == DFNT_INT8) then
         Istatus_Sum = sfsnatt(Sds%Id_Output, "valid_range", Sds%Data_Type_HDF, 2, int(Sds%valid_range,kind=int1)) + Istatus_Sum
      elseif (Sds%Data_Type_HDF == DFNT_INT16) then
         Istatus_Sum = sfsnatt(Sds%Id_Output, "valid_range", Sds%Data_Type_HDF, 2, int(Sds%valid_range,kind=int2)) + Istatus_Sum
      elseif (Sds%Data_Type_HDF == DFNT_FLOAT32) then
         Istatus_Sum = sfsnatt(Sds%Id_Output, "valid_range", Sds%Data_Type_HDF, 2, Sds%valid_range) + Istatus_Sum
     endif
...
!      Istatus_Sum = sfsnatt(Sds%Id_Output, "SCALED_MISSING", DFNT_INT32, 1, Sds%Scaled_Missing) + Istatus_Sum
     ...
     if (Sds%Unscaled_Missing .NEfp. No_Attribute_Missing_Value) then
         ...
         elseif (Sds%Data_Type_HDF == DFNT_INT16) then
           Istatus_Sum = sfsnatt(Sds%Id_Output, "_FillValue", Sds%Data_Type_HDF, 1, int(Sds%Fill_Value,kind=int2)) + Istatus_Sum
         elseif (Sds%Data_Type_HDF == DFNT_FLOAT32) then
           Istatus_Sum = sfsnatt(Sds%Id_Output, "_FillValue", Sds%Data_Type_HDF, 1, Sds%Fill_Value) + Istatus_Sum
         endif
     endif
