...
!----------------------------------------------------------------------
  ...
  public:: COMPUTE_BT_NOISE_TABLE
  public:: COMPUTE_BT_NOISE_VECTOR
  public:: COMPUTE_BT_NOISE
  ...
  ! Noise Tables
  !-------------------------------------------------------------------------------
  integer, parameter, private:: Nchan_Noise = 44
  integer, parameter, private:: N_T_Vector = 161
  real (kind=int4), parameter, private:: T_Vector_min = 180.0,  &
                                         delta_T_Vector = 1.0
  real, save, dimension(N_T_Vector) :: T_Vector, Noise_Vector
  real, parameter, private :: T_Noise_Ref = 300.0
  real, save, private, dimension(Nchan_Noise) :: Noise_Ref
  real, save, private, dimension(Nchan_Noise,N_T_Vector) :: Chan_Noise_Table

  contains 

!-------------------------------------------------------
! extract correct value from table 
!-------------------------------------------------------
function COMPUTE_BT_NOISE(Chan_Idx, T) result(NEDT)
   integer, intent(in):: Chan_Idx
   real, intent(in):: T
   real::NEDT
   integer:: T_Idx
   T_Idx = (T - T_Vector_Min) / delta_T_Vector + 1
   T_Idx = min(max(T_Idx,1),N_T_Vector)
   NEDT = Chan_Noise_Table(Chan_Idx,T_Idx)
end function COMPUTE_BT_NOISE

!----------------------------------------------------------------------
!  compute the table
!----------------------------------------------------------------------
subroutine COMPUTE_BT_NOISE_TABLE(WMO_Id, &
                                  Chan_Idx_038um, Chan_On_038um, &
                                  Chan_Idx_062um, Chan_On_062um, &
                                  Chan_Idx_067um, Chan_On_067um, &
                                  Chan_Idx_073um, Chan_On_073um, &
                                  Chan_Idx_085um, Chan_On_085um, &
                                  Chan_Idx_097um, Chan_On_097um, &
                                  Chan_Idx_104um, Chan_On_104um, &
                                  Chan_Idx_110um, Chan_On_110um, &
                                  Chan_Idx_120um, Chan_On_120um, &
                                  Chan_Idx_133um, Chan_On_133um, &
                                  Chan_Idx_136um, Chan_On_136um, &
                                  Chan_Idx_139um, Chan_On_139um, &
                                  Chan_Idx_142um, Chan_On_142um)

  integer, intent(in):: WMO_Id
  integer, intent(in):: Chan_Idx_038um, Chan_On_038um, &
                        Chan_Idx_062um, Chan_On_062um, &
                        Chan_Idx_067um, Chan_On_067um, &
                        Chan_Idx_073um, Chan_On_073um, &
                        Chan_Idx_085um, Chan_On_085um, &
  ...
  integer:: Chan_Idx, T_Idx
  real:: Rad, Rad_Noise

  Noise_Ref = MISSING_VALUE_REAL4
  Noise_Ref(20:25) = 0.2
  Noise_Ref(27:38) = 0.2

  do T_Idx = 1, N_T_Vector
     T_Vector(T_Idx) = T_Vector_Min + delta_T_Vector * (T_Idx - 1)
  enddo

  Chan_Noise_Table = MISSING_VALUE_REAL4

  if (Chan_On_038um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_038um)
    Chan_Noise_Table(Chan_Idx_038um,:) = Noise_Vector
  endif

  if (Chan_On_062um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_062um)
    Chan_Noise_Table(Chan_Idx_062um,:) = Noise_Vector
  endif

  if (Chan_On_067um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_067um)
    Chan_Noise_Table(Chan_Idx_067um,:) = Noise_Vector
  endif

  if (Chan_On_073um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_073um)
    Chan_Noise_Table(Chan_Idx_073um,:) = Noise_Vector
  endif

  if (Chan_On_085um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_085um)
    Chan_Noise_Table(Chan_Idx_085um,:) = Noise_Vector
  endif

  if (Chan_On_097um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_097um)
    Chan_Noise_Table(Chan_Idx_097um,:) = Noise_Vector
  endif

  if (Chan_On_104um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_104um)
    Chan_Noise_Table(Chan_Idx_104um,:) = Noise_Vector
  endif

  if (Chan_On_110um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_110um)
    Chan_Noise_Table(Chan_Idx_110um,:) = Noise_Vector
  endif

  if (Chan_On_120um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_120um)
    Chan_Noise_Table(Chan_Idx_120um,:) = Noise_Vector
  endif

  if (Chan_On_133um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_133um)
    Chan_Noise_Table(Chan_Idx_133um,:) = Noise_Vector
  endif

  if (Chan_On_136um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_136um)
    Chan_Noise_Table(Chan_Idx_136um,:) = Noise_Vector
  endif

  if (Chan_On_139um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_139um)
    Chan_Noise_Table(Chan_Idx_139um,:) = Noise_Vector
  endif

  if (Chan_On_142um == 1) then
    call COMPUTE_BT_NOISE_VECTOR(Chan_Idx_142um)
    Chan_Noise_Table(Chan_Idx_142um,:) = Noise_Vector
  endif

end subroutine COMPUTE_BT_NOISE_TABLE
!----------------------------------------------------------------------
!----------------------------------------------------------------------

subroutine COMPUTE_BT_NOISE_VECTOR(Chan_Idx)

    integer, intent(in):: Chan_Idx
    real:: Rad_Noise, Rad
    integer:: T_Idx

    Rad_Noise  = PLANCK_RAD_FAST(Chan_Idx,T_Noise_Ref)- PLANCK_RAD_FAST(Chan_Idx,T_Noise_Ref - Noise_Ref(Chan_Idx))
    do T_Idx = 1, N_T_Vector
      Rad = PLANCK_RAD_FAST(Chan_Idx, T_Vector(T_Idx))
      Noise_Vector(T_Idx) = PLANCK_TEMP_FAST(Chan_Idx, Rad + Rad_Noise) - T_Vector(T_Idx)
    enddo

end subroutine COMPUTE_BT_NOISE_VECTOR
!----------------------------------------------------------------------
