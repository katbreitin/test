...
!---------------------------------------------------------------------- 
 subroutine AWG_CLOUD_HEIGHT_BRIDGE()
   ...
   !-----------------------------------------------------------------------
   One_Byte_Temp = 0
   where(Output%Pc == MISSING_VALUE_REAL4)
      One_Byte_Temp = 1
   endwhere
   call COMPUTE_MEDIAN_SEGMENT(Output%Pc,One_Byte_Temp,1, &
...
!--------------------------------------------------------
 subroutine SET_INPUT()
   ...
   Input%Tc_Ap => ACHA%Tc_Ap
   Input%Ec_Ap => ACHA%Ec_Ap
   Input%Beta_Ap => ACHA%Beta_Ap
   Input%Ice_Prob_Ap => ACHA%Ice_Prob_Ap
   Input%Lower_Tc_Ap => ACHA%Lower_Tc_Ap
   Input%Tc_Ap_Uncer => ACHA%Tc_Ap_Uncer
   Input%Ec_Ap_Uncer => ACHA%Ec_Ap_Uncer
   Input%Beta_Ap_Uncer => ACHA%Beta_Ap_Uncer
   Input%Ice_Prob_Ap_Uncer => ACHA%Ice_Prob_Ap_Uncer
   Input%Lower_Tc_Ap_Uncer => ACHA%Lower_Tc_Ap_Uncer
   Input%QRNN_Pc => QRNN_CTP
   Input%QRNN_Pc_Uncer => QRNN_CTP_Uncer
...
!----------------------------------------------------------------------
 ...
 subroutine LHP_CHN_CHECK ()
   ...
   !--- Nullify LHP_THRESH if sounder (aka fusion) data is used
   do N_Chan = 1, Nchan_Clavrx
