...
!      08/29/2017: add HID_T kind for space in global declarations  (AW)
!                   this seemed to be needed for hdf5 1.10.1 intel for ifort 16.0.1
!--------------------------------------------------------------------------------------
...
! *******************************************************************
  ...
  INTEGER(HID_T)      :: dspace ! 2017/08/29 AW: added HID_T
...
CONTAINS
   ...
   subroutine H5_DATASET_DIMENSIONS  &
      ...
      ltype = int(OpenLevels_type(NLevels),4)
...
! *******************************************************************
  ...
  SUBROUTINE H5ReadString1D      &
    ...
    do indx=1,int(dims(1),4)
       iend=ibeg + int(strlen,4) - 1        ! end index along string at index idts
       if (iend.le.deflen) then  ! still fully in current idts
          ...
          idts=idts+1
          ibeg=int(strlen,4)-(deflen-ibeg+1)+1
       endif
