    ...
    case(DIM_2)
      ...
      case(DCLASS_REAL)
        if (  present (start_inp)) then
          ...
          counts(2) = min(counts(2),dims(2)-start_inp(2)-1)
          call H5ReadDataset ( h5_file, sds_name(1),offs,counts, dataset_2d )
          dims_this(2) = counts(2)
        else
          call H5ReadDataset ( h5_file, sds_name(1), dataset_2d )
        end if

        allocate ( sdata(1) % data % r4values(dims_this(1)*dims_this(2)))
        sdata(1) % data % r4values = reshape(dataset_2d, (/dims_this(1)*dims_this(2)/) )
      ...
      case(DCLASS_INT)
        ...
        dims_this = shape(dataset_2d_i)
        n_elem = size(dataset_2d_i)

        allocate ( sdata(1) % data % i4values(n_elem))
        sdata(1) % data % i4values = reshape(dataset_2d_i, (/n_elem/) )
        sdata(1) % data % nval = n_elem
      ...
      sdata(1) % data % dimsize(1) = dims_this(1)
      sdata(1) % data % dimsize(2) = dims_this(2)
      sdata(1) % data % rank = 2
