! $Id: cloud_tau_re_solar_1dvar.f90 3502 2019-09-10 13:07:45Z heidinger $
...
! real(kind=real4):: tc           !cloud top temperature
  ...
  real(kind=real4):: Ref_065um_Clear  !clear ch2 reflectance
...
!--- local variables
  ...
  real(kind=real4):: Alb_Sfc_Dark_065um
  real(kind=real4):: Alb_Sfc_Dark_160um
  real(kind=real4):: Alb_Sfc_Dark_375um
  real(kind=real4):: Alb_Sfc_White_065um
  real(kind=real4):: Alb_Sfc_White_160um
  real(kind=real4):: Alb_Sfc_White_375um
  ...
  integer:: Sensor_Chan_Idx_065um
  integer:: Sensor_Chan_Idx_160um
  integer:: Sensor_Chan_Idx_220um
  integer:: Sensor_Chan_Idx_375um

  !--- parameters for surface albedo
  real, parameter:: log10tau_white_sfc = 0.6  
  real, parameter:: log10tau_dark_sfc = -0.2  
...
!--------------------------------------------------------------------------
 ...
 if (imode == 0) then
    print *, 'No solar_tau_reff calculations possible ', Dcomp_Mode, Sensor%Chan_On_Flag_Default(6), &
              Sensor%Chan_On_Flag_Default(7), Sensor%Chan_On_Flag_Default(20)
 endif
...
!endif
  ...
  !----------------------------------------------------------------------
  Lut_Path = TRIM(ancil_data_dir)//"/static/luts/cld/"
...
!Sensor%CLAVRx_Chan_Map = (/3,1,2,26,6,7,20,37,27,28,29,30,38,31,32,33/)
       ...
       !--- get chan_idx of sensor
       Sensor_Chan_Idx_065um  = minloc(abs(Sensor%CLAVRx_Chan_Map - Chan_Idx_065um),1)
       Sensor_Chan_Idx_160um  = minloc(abs(Sensor%CLAVRx_Chan_Map - Chan_Idx_160um),1)
       Sensor_Chan_Idx_375um  = minloc(abs(Sensor%CLAVRx_Chan_Map - Chan_Idx_375um),1)

print *, "Sensor_Chan_Idx_065um = ", Sensor_Chan_Idx_065um
print *, "Sensor_Chan_Idx_160um = ", Sensor_Chan_Idx_160um
print *, "Sensor_Chan_Idx_375um = ", Sensor_Chan_Idx_375um

       !--- Read channel 1 tables
       if (Sensor%Chan_On_Flag_Default(Chan_Idx_065um) == sym%YES) then
          call READ_TABLES(Algo_Idx,Chan_Idx_065um,Sensor_Chan_Idx_065um,Lut_Path)
       endif

       !--- Read 1.60 um channel 
       if (Sensor%Chan_On_Flag_Default(Chan_Idx_160um) == sym%YES) then
           call READ_TABLES(Algo_Idx,Chan_Idx_160um,Sensor_Chan_Idx_160um,Lut_Path)
       endif

       !--- Read 3.75 um channel 
       if (Sensor%Chan_On_Flag_Default(Chan_Idx_375um) == sym%YES) then
           call READ_TABLES(Algo_Idx,Chan_Idx_375um,Sensor_Chan_Idx_375um,Lut_Path)
       endif
  ...
  line_loop_1: Do Line_Idx= 1, Image%Number_Of_Lines_Read_This_Segment
    element_loop_1: Do Elem_Idx= 1, Image%Number_Of_Elements
      ...
      xnwp = NWP_PIX%i_nwp(Elem_Idx,Line_Idx)            !nwp longitude cell
      ynwp = NWP_PIX%j_nwp(Elem_Idx,Line_Idx)            !nwp latitude cell
      itropo = NWP%Tropo_Level(xnwp,ynwp)  !nwp level associated with tropopause
      ivza = Zen_Idx_Rtm(Elem_Idx,Line_Idx)         !viewing zenith angle bin
      isfc = NWP%Sfc_Level(xnwp,ynwp)      !first nwp level above surface
      ...
      Ref_std = ch(1)%Ref_Toa_Std(Elem_Idx,Line_Idx)   !measure of reflectance variability
      Ref_mean = ch(1)%Ref_Toa_Mean(Elem_Idx,Line_Idx)   !measure of reflectance variability
      Ref_var = 0.0
      ...
      plev => NWP%P_Std
      tlev => NWP%T_Prof(:,xnwp,ynwp)
      zlev => NWP%Z_Prof(:,xnwp,ynwp)
      Tpwlev => NWP%Tpw_Prof(:,xnwp,ynwp)

      Tpw  = Nwp_Pix%Tpw(Elem_Idx,Line_Idx)                   !total column pw
      Tsfc  = Nwp_Pix%Tsfc(Elem_Idx,Line_Idx)                 !surface temperature
...
!----------------------------------------------------------------------
   ...
   !----------------------------------------------------------------------
     ...
     !-- above cloud ch1
     Tau_Aer =  Solar_Rtm%Tau_Aer(1) * (P_cloud / NWP%Psfc(xnwp,ynwp))**2
     Tau_Ray =  Solar_Rtm%Tau_Ray(1) * (P_cloud / NWP%Psfc(xnwp,ynwp))
     ...
     !-- above cloud
     Tau_Aer =  Solar_Rtm%Tau_Aer(6) * (P_cloud / Nwp%Psfc(xnwp,ynwp))**2
     Tau_Ray =  Solar_Rtm%Tau_Ray(6) * (P_cloud / Nwp%Psfc(xnwp,ynwp))
...
!----------------------------------------------------------------------
! set white sky surface reflectance (0-1)
!----------------------------------------------------------------------
Alb_Sfc_White_065um = ch1_Sfc_Alb_UMD(Sfc%Sfc_Type(Elem_Idx,Line_Idx)) / 100.0
Alb_Sfc_White_160um = ch6_Sfc_Alb_UMD(Sfc%Sfc_Type(Elem_Idx,Line_Idx)) / 100.0
Alb_Sfc_White_375um = ch20_Sfc_Alb_UMD(Sfc%Sfc_Type(Elem_Idx,Line_Idx)) / 100.0
  ...
  if (Modis_Clr_Alb_Flag == sym%YES) then
   if (Sensor%Chan_On_Flag_Default(1) == sym%YES) then
    if (ch(1)%Sfc_Ref_White_Sky(Elem_Idx,Line_Idx) > 0.0)  then
       Alb_Sfc_White_065um = ch(1)%Sfc_Ref_White_Sky(Elem_Idx,Line_Idx)/100.0
    endif
   endif
   if (Sensor%Chan_On_Flag_Default(6) == sym%YES) then
    if (ch(6)%Sfc_Ref_White_Sky(Elem_Idx,Line_Idx) > 0.0)  then
       Alb_Sfc_White_160um =ch(6)%Sfc_Ref_White_Sky(Elem_Idx,Line_Idx)/100.0
    endif
   endif
  endif
 endif

!--- use sfc emissivities for surface reflectance for 3.75 micron
 if (Sensor%Chan_On_Flag_Default(20) == sym%YES) then
  if ((Emiss_Sfc_375um > 0.0) .and. (Sfc%Sfc_Type(Elem_Idx,Line_Idx) > 0))  then   
            Alb_Sfc_White_375um = 1.0 - Emiss_Sfc_375um
  endif  
 endif  

!--------------------------------------------------------------------------
! set dark sky albedoes (0-1)
!--------------------------------------------------------------------------
if (Sfc%Sfc_Type(ELem_Idx,Line_Idx) == 0) then
   Alb_Sfc_Dark_065um = 0.02
   Alb_Sfc_Dark_160um = 0.01
   Alb_Sfc_Dark_375um = 0.00
else
   Alb_Sfc_Dark_065um = Alb_Sfc_White_065um
   Alb_Sfc_Dark_160um = Alb_Sfc_White_160um
   Alb_Sfc_Dark_375um = Alb_Sfc_White_375um
endif
...
!print *, "apriori tau = ", y(1), Alb_Sfc_Dark_065um, 10.0**x_ap(1)
    ...
    !--- recompute clear sky scatter with cloud albedo terms
     call COMPUTE_CLEAR_SKY_SCATTER(Tau_aer, &
      ...
      Ref_065um_Clear = Ref_Ss_065um + Trans_Sfc_065um*Alb_Sfc_Dark_065um
...
if (diag_output == sym%YES) then
   ...
   print *, "sfc ref = ", Alb_Sfc_White_065um,Alb_Sfc_White_160um,Alb_Sfc_White_375um
...
retrieval_loop: do
 ...
 !--- determine weight between dark and white sky albedo for forward model
 if (log10tau .ge. log10tau_white_sfc) then
    Alb_Sfc_065um  = Alb_Sfc_White_065um
    Alb_Sfc_160um  = Alb_Sfc_White_160um
    Alb_Sfc_375um  = Alb_Sfc_White_375um
 elseif (log10tau .le. log10tau_dark_sfc) then
    Alb_Sfc_065um  = Alb_Sfc_Dark_065um
    Alb_Sfc_160um  = Alb_Sfc_Dark_160um
    Alb_Sfc_375um  = Alb_Sfc_Dark_375um
 else
    Alb_Sfc_065um  = Alb_Sfc_Dark_065um + log10tau * &
                     (Alb_Sfc_White_065um - Alb_Sfc_Dark_065um) / &
                     (log10tau_white_sfc - log10tau_dark_sfc)
    Alb_Sfc_160um  = Alb_Sfc_Dark_160um + log10tau * &
                     (Alb_Sfc_White_160um - Alb_Sfc_Dark_160um) / &
                     (log10tau_white_sfc - log10tau_dark_sfc)
    Alb_Sfc_375um  = Alb_Sfc_Dark_375um + log10tau * &
                     (Alb_Sfc_White_375um - Alb_Sfc_Dark_375um) / &
                     (log10tau_white_sfc - log10tau_dark_sfc)
 endif
...
!=======================================================================
subroutine READ_TABLES(Algo_Idx,Chan_Idx,Sensor_Chan_Idx, Lut_Path)

 integer(kind=int4), intent(in):: Algo_Idx,Chan_Idx, Sensor_Chan_Idx
 character(len=*), intent(in):: Lut_Path
      ...
      if (Chan_Idx <= 20) then
         ...
         Cld_Ref(Algo_Idx)%channel(Chan_Idx)%flag = sym%YES
          ...
          call READ_CLD_REF_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx, WATER_PHASE)

          !--- set flag and read in ice phase tables
          Cld_Ref(Algo_Idx)%channel(Chan_Idx)%phase(ICE_PHASE)%flag = sym%YES
          call READ_CLD_REF_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx, ICE_PHASE)

          !--- set flag and read in mixed phase tables
          Cld_Ref(Algo_Idx)%channel(Chan_Idx)%phase(MIXED_PHASE)%flag = sym%YES
          call READ_CLD_REF_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx, MIXED_PHASE) 
      ...
      if (Chan_Idx >= 20) then
         ...
         Cld_Ems(Algo_Idx)%channel(Chan_Idx)%flag = sym%YES
          ...
          call READ_CLD_EMS_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx,WATER_PHASE)

          !--- set flag and read in ice phase tables
          Cld_Ems(Algo_Idx)%channel(Chan_Idx)%phase(ICE_PHASE)%flag = sym%YES
          call READ_CLD_EMS_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx,ICE_PHASE)

          !--- set flag and read in mixed phase tables
          Cld_Ems(Algo_Idx)%channel(Chan_Idx)%phase(MIXED_PHASE)%flag = sym%YES
          call READ_CLD_EMS_LUT_STRUC(Lut_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx,MIXED_PHASE)
...
!=======================================================================
 subroutine READ_CLD_REF_LUT_STRUC(Ancil_Data_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx, Phase_Idx)

  integer, intent(in):: Algo_Idx,Chan_Idx,Sensor_Chan_Idx
  ...
  if (Sensor_Chan_Idx < 10) then
     write(Chan_String,fmt="(I1)") Sensor_Chan_Idx
  else
     write(Chan_String,fmt="(I2)") Sensor_Chan_Idx
  endif
...
!
!=======================================================================
 subroutine READ_CLD_EMS_LUT_STRUC(Ancil_Data_Path,Algo_Idx,Chan_Idx,Sensor_Chan_Idx,Phase_Idx)
